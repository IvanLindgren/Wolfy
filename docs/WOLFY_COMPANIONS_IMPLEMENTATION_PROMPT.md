...
9. ИИ-функции всегда помечены `Beta` и рядом есть короткое предупреждение: `ИИ может ошибаться.` Число дневного лимита в интерфейс не пишется: оно живёт в `readingai.DailyLimit`, а читатель видит остаток из ответа сервера.
...
Под действиями ИИ один раз показывай: `ИИ может ошибаться.` Не дублируй предупреждение под каждой кнопкой и не повторяй в нём число лимита — оно разойдётся с сервером молча.
...
Ручные opinion, question и recap используют общий дневной лимит `readingai.DailyLimit` на пользователя. В UI показывай `remaining` из ответа, а не само число лимита.

...
/// Основы-кандидаты для глагольного окончания — то же, что [`candidates`], но
/// доступное соседнему модулю.
///
/// Разбор форм глагола ([`super::verbs`]) обязан отыгрывать орфографию назад
/// теми же правилами, что и карточка слова: иначе «stopped» в карточке сведётся
/// к «stop», а в разборе времени — нет, и читатель увидит два разных ответа на
/// один и тот же вопрос.
pub(super) fn verb_stems(suffix: &str, stem: &str) -> Vec<String> {
    candidates(suffix, stem)
}

...
pub struct AppSettings {
    ...
    /// страница книги выглядит как страница книги и ничего не обещает. Поэтому
    /// читалка один раз подсказывает это сама и навсегда замолкает, как только
    /// подсказкой воспользовались.
    ///
    /// Настройка, а не местная отметка: научился человек, а не устройство, и
    /// переучивать его на планшете незачем.
    #[serde(default)]
    pub word_tap_seen: bool,
...
impl AppSettings {
    ...
    pub fn replaced_by(&self, incoming: &AppSettings) -> AppSettings {
        AppSettings {
            // Пройденное не проходят заново. Обе отметки ходят только из
            // false в true, поэтому сливаются ИЛИ, а не заменой: иначе
            // устройство, где читатель ещё не пробовал, приехало бы со своим
            // false и вернуло вступление и подсказку тому, кто их давно прошёл.
            //
            // Разница между ними только в цене ошибки. Подсказку про касание
            // слова можно закрыть одним нажатием, а вступление — это экран
            // поверх всего приложения, и всплывает он ровно в тот момент,
            // когда читатель поставил Wolfy на второе устройство и ждал книг.
            onboarding_seen: self.onboarding_seen || incoming.onboarding_seen,
            word_tap_seen: self.word_tap_seen || incoming.word_tap_seen,
...
mod tests {
    ...
    /// Умение не забывается ни в одну сторону синхронизации.
    ///
    /// Отметка «разбор слова уже открывали» ходит только из false в true.
    /// Заменой её сливать нельзя: планшет, на котором читатель ещё не пробовал
    /// касание, вернул бы подсказку тому, кто давно её прошёл на телефоне, — и
    /// наоборот, телефон затёр бы правду планшета.
    #[test]
    fn пройденное_не_проходят_заново_после_слияния() {
        let знающий = AppSettings {
            word_tap_seen: true,
            onboarding_seen: true,
            ..Default::default()
        };
        let незнающий = AppSettings::default();

        let затёртый = знающий.replaced_by(&незнающий);
        assert!(
            затёртый.word_tap_seen,
            "приехавшее незнание затёрло уже полученное умение"
        );
        assert!(
            затёртый.onboarding_seen,
            "второе устройство вернуло вступление тому, кто его уже прошёл"
        );

        let доехавший = незнающий.replaced_by(&знающий);
        assert!(
            доехавший.word_tap_seen,
            "умение с другого устройства не доехало"
        );
        assert!(
            доехавший.onboarding_seen,
            "пройденное вступление с другого устройства не доехало"
        );

        let чистый = незнающий.replaced_by(&незнающий);
        assert!(
            !чистый.word_tap_seen,
            "отметка появилась сама, без единого касания"
        );
        assert!(
            !чистый.onboarding_seen,
            "вступление засчиталось само, без единого экрана"
        );
    }
